...
var dao=function(){
	...
	//通过movieId查电影信息 
	this.xiangQingPage=function(client,movieId,callback){
		client.query("SELECT * FROM movie WHERE movie_id=?",[movieId],function(error,result){
			callback(result[0]);
		})
	};
	
	this.selectFangYing=function(client,movieId,callback){
		client.query("SELECT * FROM changci WHERE movie_id=?",[movieId],function(error,result){
			callback(result);
		})
	}
	...
	//添加场次
	this.insertChangCi=function(client,movieNews,callback){
		client.query("INSERT INTO changci (changci_id,movie_id,TIME,paice,fangyingting) VALUES(?,?,?,?,?)",[uuid.v4(),movieNews,"2017-8-21 12:26","45",7],function(error,result){
			callback(result.affectedRows);
		});
	}
	//删除单个场次
	this.deleteOneChangCi = function(client, id, callback) {
		client.query("DELETE FROM changci WHERE changci_id = ?", [id], function(error, result) {
			callback(result.affectedRows);
		})
	};
